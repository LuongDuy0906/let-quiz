        ...
        try {
            ...
            const countdownTimer = setInterval(async () => {
                if (countdown > 0) {
                    ...
                    let optionsData = currentQuestion.options.map((opt: any) => ({ id: opt._id, text: opt.content }));
        ...
        try {
            ...
            if (rawQuestion && rawRoomData) {
                ...
                if (isCorrect) {
                    ...
                    const timeRatio = safeTimeElapsed / durationMs;
                    const scoreMultiplier = 1 - (timeRatio * (1 - minRatio));

                    finalScore = Math.round(maxScore * scoreMultiplier);
                } else {
                    finalScore = 0;
                }
            }
